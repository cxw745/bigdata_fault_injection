...
INJECT_MODE = os.environ.get("TASK_FAIL_MODE", "task_id")
INJECT_TASK_IDS = os.environ.get("TASK_FAIL_TASK_IDS", "0,5,10")
INJECT_RATIO = float(os.environ.get("TASK_FAIL_RATIO", "0.2"))
INJECT_PROBABILITY = float(os.environ.get("TASK_FAIL_PROBABILITY", "0.2"))
...
def inject_failure():
    ...
    if should_inject:
        task_id = get_task_id()
        raise RuntimeError(f"[TASK_FAIL] Injected failure for task_id={task_id}")
